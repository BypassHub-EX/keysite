...
const bindings = fs.existsSync(BINDINGS_FILE) ? JSON.parse(fs.readFileSync(BINDINGS_FILE)) : {};
...
function loadKeys() {
  ...
  return fs.readFileSync(KEYS_FILE, "utf8").split(/\r?\n/).map(k => k.trim()).filter(k => k.length > 0);
}
...
app.get("/nmt.script", (_req, res) => {
  if (!fs.existsSync(SCRIPT_FILE)) return res.status(500).send("Script missing.");
  res.type("text/plain");
  return res.sendFile(SCRIPT_FILE);
});

// === Allow public file (keys.txt) ===
