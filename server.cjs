...
// ==== CONFIG ====
const ADMIN_PATH  = process.env.ADMIN_PATH || "/super-secret-admin-492xya"; // 🔑 change this
...
function takeRandomKey(){
  ...
  const key = keyPool[i];
  keyPool.splice(i,1); // remove after use
  return key;
}
setInterval(()=>{
  const now=Date.now();
...
code{display:block;background:#0e131a;border:1px solid #212833;padding:14px;border-radius:10px;margin-bottom:16px;font-family:monospace}
...
// ==== SCRIPT DELIVERY ====
// executor-only, no key param required
